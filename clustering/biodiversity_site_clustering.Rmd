...
## ----------------------------
## Clusters per AEZ per method/radius (incl. zero-cluster AEZs)
## ----------------------------


# get the full AEZ roster from the first bundle (or from your raw sites table if you prefer)
...
# Optional: flag AEZ-method-radius combos with zero clusters
zero_cluster_flags <- clusters_by_aez_tbl |>
  filter(n_clusters == 0L)

zero_cluster_flags

# write outputs
write_csv(clusters_by_aez_tbl,  "clusters_by_AEZ_long.csv")
write_csv(clusters_by_aez_wide, "clusters_by_AEZ_wide.csv")
write_csv(zero_cluster_flags,   "clusters_by_AEZ_zero_flags.csv")
...
# amend - map to visualize the clusters with OV, colored - minmax to transform it from logged to unlogged - to see if there is Ov variance over time - do by AEZ; this is the best way to differentiate between the clustering approaches
