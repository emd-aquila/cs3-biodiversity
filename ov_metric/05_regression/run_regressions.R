...
regression_dir <- if (length(file_arg) > 0) {
...
if (!dir.exists(code_dir)) {
  stop("Regression code directory not found: ", code_dir, call. = FALSE)
}
          ...
          for (defor_approach in names(defor_approach_specs)) {
            ...
            for (regression_model in regression_models) {
              ...
              for (i in seq_len(nrow(regression_run_grid))) {
                cluster_database_i <- regression_run_grid$cluster_database[i]
                ...
                log_verbose("  cluster_database: ", cluster_database_i)
                ...
                set_regression_run_paths(
                  cluster_database = cluster_database_i,
