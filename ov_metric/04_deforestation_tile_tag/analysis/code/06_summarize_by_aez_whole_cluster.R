...
# for the whole-cluster workflow
# =====================================================
...
if (nrow(cluster_tag_status) == 0) {
  ...
  defor_pair_summary_by_aez <- tibble()
  aez_summary <- tibble()
  ...
  assert_has_cols(
    cluster_year_pairs,
    c("AEZ", "cluster_id", "year_start", "year_final"),
  ...
  cluster_counts_by_aez <- cluster_tag_status %>%
    ...
    summarise(
      ...
      clusters_tagged_to_defor_tile = sum(tagged_any_tile, na.rm = TRUE),
      mean_n_matched_defor_tiles = dplyr::if_else(
        any(tagged_any_tile),
        mean(n_matched_tiles[tagged_any_tile], na.rm = TRUE),
        0
      ),
  ...
  if (nrow(cluster_deltas) == 0) {
    defor_pair_summary_by_aez <- tibble(
      AEZ = character(),
      pairs_tagged_to_defor_tile = integer(),
      n_inverse_change_defor_tag = integer(),
      n_aligned_change_defor_tag = integer()
    )
  } else {
    defor_pair_summary_by_aez <- cluster_deltas %>%
      mutate(
      ...
      group_by(AEZ) %>%
      summarise(
        pairs_tagged_to_defor_tile = n(),
        n_inverse_change_defor_tag = sum(inverse_change, na.rm = TRUE),
        n_aligned_change_defor_tag = sum(aligned_change, na.rm = TRUE),
        .groups = "drop"
      )
  }
  ...
  aez_summary <- all_aez %>%
    ...
    left_join(defor_pair_summary_by_aez, by = "AEZ") %>%
    mutate(
      across(
        c(
          total_clusters,
          total_cluster_pairs,
          clusters_tagged_to_defor_tile,
          pairs_tagged_to_defor_tile,
          n_inverse_change_defor_tag,
          n_aligned_change_defor_tag,
          clusters_tagged_to_ha_tile
        ),
        ~ tidyr::replace_na(.x, 0)
      ),
      mean_n_matched_defor_tiles = replace_na(mean_n_matched_defor_tiles, 0),
      mean_n_matched_ha_tiles = replace_na(mean_n_matched_ha_tiles, 0),
      share_inverse_change_defor_tag = if_else(
        pairs_tagged_to_defor_tile > 0,
        n_inverse_change_defor_tag / pairs_tagged_to_defor_tile,
        0
      ),
      share_aligned_change_defor_tag = if_else(
        pairs_tagged_to_defor_tile > 0,
        n_aligned_change_defor_tag / pairs_tagged_to_defor_tile,
        0
      ),
      buffer_km = current_buffer_km,
      cluster_database = if (exists("current_cluster_database")) current_cluster_database else NA_character_,
    ...
    dplyr::select(
      ...
      clusters_tagged_to_defor_tile,
      pairs_tagged_to_defor_tile,
      mean_n_matched_defor_tiles,
      n_inverse_change_defor_tag,
      n_aligned_change_defor_tag,
      share_inverse_change_defor_tag,
      share_aligned_change_defor_tag,
      clusters_tagged_to_ha_tile,
