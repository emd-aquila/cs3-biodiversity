...
if (nrow(cluster_buffer_this) == 0) {
  ...
  cluster_year_pairs <- build_year_pair_ov_table(analysis_unit_year_ov_this, available_ov_score_specs)
  ...
  # -----------------------
  # Restrict regression inputs to clusters tagged to >=1 deforestation tile.
  ...
  interval_defor_tagged_defor <- cluster_pairs_tagged_defor %>%
    mutate(
      cutoff_year_end = compute_cutoff_interval_end(year_t1, year_t2, year_gap),
      lagged_year_start = year_t1 - 1L,
      lagged_year_end = year_t2 - 1L
    ) %>%
    dplyr::select(
      AEZ, cluster_id, buffer_km, year_t1, year_t2, year_gap,
    ...
    filter(year >= lagged_year_start, year <= year_t2) %>%
    group_by(AEZ, cluster_id, buffer_km, year_t1, year_t2) %>%
    summarise(
      delta_defor_ha_total_raw = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(year_t2), defor_ha_total_raw, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_total_avg = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(year_t2), defor_ha_total_avg, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_total_rel_pct = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(year_t2), defor_ha_total_rel_pct, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_crops_raw = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(year_t2), defor_ha_crops_raw, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_crops_avg = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(year_t2), defor_ha_crops_avg, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_crops_rel_pct = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(year_t2), defor_ha_crops_rel_pct, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_total_raw_cutoff = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(cutoff_year_end), defor_ha_total_raw, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_total_avg_cutoff = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(cutoff_year_end), defor_ha_total_avg, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_total_rel_pct_cutoff = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(cutoff_year_end), defor_ha_total_rel_pct, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_crops_raw_cutoff = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(cutoff_year_end), defor_ha_crops_raw, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_crops_avg_cutoff = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(cutoff_year_end), defor_ha_crops_avg, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_crops_rel_pct_cutoff = sum(
        dplyr::if_else(year >= first(year_t1) & year <= first(cutoff_year_end), defor_ha_crops_rel_pct, 0),
        na.rm = TRUE
      ),
      delta_defor_ha_total_raw_lagged = dplyr::if_else(
        sum(year >= first(lagged_year_start) & year <= first(lagged_year_end), na.rm = TRUE) ==
          first(year_t2 - year_t1 + 1L),
      ...
      n_defor_years = sum(year >= first(year_t1) & year <= first(year_t2), na.rm = TRUE),
      n_defor_years_cutoff = sum(year >= first(year_t1) & year <= first(cutoff_year_end), na.rm = TRUE),
  ...
  assert_has_cols(
    ...
    c(
      "AEZ", "cluster_id", "buffer_km",
      "cluster_database", "cluster_method", "cluster_radius_km",
  ...
  if (exists("current_cluster_stub")) {
    message("  cluster run: ", current_cluster_run_label)
  }
