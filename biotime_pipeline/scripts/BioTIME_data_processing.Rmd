...
# 2. Load data
```{r}
# Mini-data batch for testing
# small_biotime <- biotime_data[1:100000, ]
# saveRDS(small_biotime, file = "../data/biotime_test_subset.Rds")
# biotime_test_subset <- readRDS("../data/biotime_test_subset.Rds")
# biotime_data_subset <- as.data.table(biotime_test_subset)
...
# Filter & build full_binomial fast
...
```{r}
resolve_bird_names <- function(dt, species_column = "valid_name", bird_lookup_path = "../data/IBP-AOS-LIST24.csv") {

  bird <- fread(bird_lookup_path, select = c("SPEC", "SPEC6", "COMMONNAME", "SCINAME"), fill = TRUE)
  bird[, `:=`(
...
```{r}
decode_plant_codes <- function(dt, species_column = "valid_name", plant_lookup_path = "../data/plant_code_lookup.csv") {

  plant_lookup <- fread(plant_lookup_path)
...
resolve_species_names <- function(dt, 
                                   ...
                                   bird_lookup_path = "../data/IBP-AOS-LIST24.csv",
                                   plant_lookup_path = "../data/plant_code_lookup.csv",
...
fwrite(data.table(valid_name = unresolved), "../data/biotime_unresolved_entries.csv")

# 2. Export resolved names
resolved <- unique(dt[!is.na(resolution_method), .(valid_name, resolved_name, resolution_method)])
fwrite(resolved, "../data/biotime_resolved_entries.csv")
...
fwrite(result, "../data/biotime_unique_entries.csv")
```