...
buffer_km_targets <- c(1)
target_aez_values <- "AEZ3"
single_tile_collapse_mode_target <- "keep_single_tile_clusters"
save_outputs <- TRUE
...
run_ov_over_time_plot <- TRUE
run_brazil_exploration <- TRUE
brazil_explore_buffer_km <- 1
brazil_explore_n_clusters <- 10L
...
set_starting_ov_adjustment_mode(starting_ov_adjustment_modes[[1]])
set_defor_approach(names(defor_approach_specs)[[1]])
...
parse_cluster_radius_km <- function(path) {
  as.numeric(stringr::str_match(path, "radius_([0-9]+(?:\\.[0-9]+)?)km")[[2]])
}

read_completed_cluster_ov_data <- function(cluster_method = cluster_method_target) {
  cluster_paths <- list.files(
    file.path(repo_root, "cluster", "output"),
    pattern = "^model_df_clustered\\.csv$",
    recursive = TRUE,
    full.names = TRUE
  )

  cluster_paths <- cluster_paths[
    grepl(paste0("/", cluster_method, "/model_df_clustered\\.csv$"), cluster_paths)
  ]

  purrr::map_dfr(
    cluster_paths,
    function(path) {
      radius_km <- parse_cluster_radius_km(path)
      cluster_data <- readr::read_csv(path, show_col_types = FALSE)
      assert_has_cols(cluster_data, c("AEZ", "cluster_id", "year", "ov_score"), basename(path))
      ...
      cluster_data %>%
        ...
        filter(!is.na(aez), !is.na(cluster_id), !is.na(year), is.finite(ov)) %>%
        group_by(radius_km, aez, cluster_id, year) %>%
        summarise(ov = median(ov, na.rm = TRUE), .groups = "drop")
    }
  )
}

write_ov_over_time_plot <- function() {
  ov_over_time_data <- read_completed_cluster_ov_data()

  if (nrow(ov_over_time_data) == 0) {
    warning("No completed clustering outputs were available for the OV-over-time plot.")
    return(invisible(NULL))
  }

  radius_levels <- sort(unique(ov_over_time_data$radius_km))
  plot_data <- ov_over_time_data %>%
    mutate(
      radius_label = factor(
        paste0(sprintf("%.1f", radius_km), " km radius"),
        levels = paste0(sprintf("%.1f", radius_levels), " km radius")
      )
    )

  plot_obj <- ggplot(plot_data, aes(x = year, y = ov, color = aez)) +
    geom_point(alpha = 0.32, size = 0.9) +
    geom_smooth(
      aes(group = 1),
      method = "lm",
      formula = y ~ x,
    ...
    facet_wrap(~ radius_label, scales = "free_y") +
    labs(
      title = "OV over time for completed cluster-radius runs",
      subtitle = "Each point is one cluster-year median OV; colors show AEZ and black lines show overall linear trends.",
      x = "Year",
      y = "Cluster-year median OV",
      color = "AEZ"
    ) +
  ...
  readr::write_csv(
    ov_over_time_data %>%
      group_by(radius_km, year) %>%
      summarise(
        n_clusters = n(),
        median_ov = median(ov, na.rm = TRUE),
        mean_ov = mean(ov, na.rm = TRUE),
        .groups = "drop"
      ),
    file.path(data_explore_dir, "ov_over_time_summary.csv")
  )
}

write_brazil_exploration <- function() {
  brazil_output_dir <- file.path(explore_root_dir, "brazil_exploration")
  cluster_delta_path <- build_cluster_deltas_explore_path(
    brazil_explore_buffer_km,
    "year_pair_delta_ov"
  )

  if (!file.exists(cluster_delta_path)) {
    warning("Skipping Brazil exploration: missing ", cluster_delta_path)
    return(invisible(NULL))
  }

  brazil_deltas <- readr::read_csv(cluster_delta_path, show_col_types = FALSE) %>%
    normalize_cluster_deltas_schema() %>%
    filter(primary_country_id == "BRA" | primary_country_iso3 == "BRA") %>%
    mutate(
      delta_defor_treecover_share_raw = as.numeric(delta_defor_treecover_share_raw),
      delta_ov = as.numeric(delta_ov),
      year_gap = as.integer(year_gap)
    ) %>%
    filter(is.finite(delta_defor_treecover_share_raw), is.finite(delta_ov))

  if (nrow(brazil_deltas) == 0) {
    warning("Skipping Brazil exploration: no Brazil rows found in ", cluster_delta_path)
    return(invisible(NULL))
  }

  dir.create(brazil_output_dir, recursive = TRUE, showWarnings = FALSE)

  brazil_summary <- brazil_deltas %>%
    summarise(
      n_transitions = n(),
      n_clusters = n_distinct(cluster_id),
      median_year_gap = median(year_gap, na.rm = TRUE),
      correlation_delta_ov_defor = cor(delta_ov, delta_defor_treecover_share_raw),
      share_delta_ov_increasing = mean(delta_ov > 0),
      share_defor_increasing = mean(delta_defor_treecover_share_raw > 0),
      share_both_increasing = mean(delta_ov > 0 & delta_defor_treecover_share_raw > 0)
    )

  illustrative_clusters <- brazil_deltas %>%
    mutate(
      both_increasing = delta_ov > 0 & delta_defor_treecover_share_raw > 0,
      illustrative_score = abs(delta_ov) * abs(delta_defor_treecover_share_raw)
    ) %>%
    arrange(desc(both_increasing), desc(illustrative_score)) %>%
    distinct(cluster_id, .keep_all = TRUE) %>%
    slice_head(n = brazil_explore_n_clusters) %>%
    mutate(point_label = paste0("P", row_number()))

  readr::write_csv(brazil_summary, file.path(brazil_output_dir, "brazil_summary.csv"))
  readr::write_csv(illustrative_clusters, file.path(brazil_output_dir, "brazil_10_points.csv"))

  scatter_plot <- ggplot(
    brazil_deltas,
    aes(x = delta_defor_treecover_share_raw, y = delta_ov)
  ) +
    geom_hline(yintercept = 0, color = "grey55", linewidth = 0.4) +
    geom_vline(xintercept = 0, color = "grey55", linewidth = 0.4) +
    geom_point(aes(color = aez), alpha = 0.42, size = 1.6) +
    geom_smooth(method = "lm", formula = y ~ x, color = "black", se = TRUE) +
    geom_label(
      data = illustrative_clusters,
    ...
    labs(
      title = "Brazil: change in OV vs. change in deforestation exposure",
      subtitle = paste0(
        "Year-pair transitions, ", brazil_explore_buffer_km,
        " km buffer. Labels identify the ten mapped illustrative clusters."
      ),
      x = "Change in deforestation share of baseline tree cover",
      y = "Change in OV",
      color = "AEZ"
    ) +
    theme_minimal(base_size = 12) +
  ...
  save_plot_if_requested(
    scatter_plot,
    "brazil_delta_ov_vs_defor.png",
    output_dir = brazil_output_dir,
    width = 10,
    height = 7
  )

  brazil_boundary <- sf::read_sf(country_boundaries_path) %>%
    rename_with(tolower) %>%
    filter(adm0_a3 == "BRA") %>%
    st_transform(4326)

  illustrative_points <- illustrative_clusters %>%
    mutate(
      label_longitude = medoid_longitude + case_when(
        point_label == "P8" ~ -2,
        point_label %in% c("P1", "P3", "P9") ~ -2,
        point_label %in% c("P4", "P6", "P10") ~ 2,
        TRUE ~ 0
      ),
      label_latitude = medoid_latitude + case_when(
        point_label == "P8" ~ -1,
        point_label %in% c("P1", "P2", "P3", "P5") ~ 1.6,
        point_label %in% c("P6", "P9", "P10") ~ -1.6,
        TRUE ~ 0.7
      )
    ) %>%
    st_as_sf(
  ...
  map_plot <- ggplot() +
    geom_sf(data = brazil_boundary, fill = "grey95", color = "grey50", linewidth = 0.4) +
    geom_sf(
      data = illustrative_points,
      aes(color = delta_ov, size = delta_defor_treecover_share_raw),
      alpha = 0.85
    ) +
    geom_segment(
      data = sf::st_drop_geometry(illustrative_points),
      aes(
        x = medoid_longitude,
        y = medoid_latitude,
    ...
    geom_label(
      data = sf::st_drop_geometry(illustrative_points),
      aes(x = label_longitude, y = label_latitude, label = point_label),
      size = 3,
      linewidth = 0.15
    ) +
    scale_color_gradient2(low = "#b2182b", mid = "grey90", high = "#2166ac", midpoint = 0) +
    labs(
      title = "Brazil: ten illustrative cluster transitions",
      subtitle = "Color shows change in OV; point size shows deforestation exposure.",
      x = NULL,
      y = NULL,
      color = "Change in OV",
      size = "Deforestation share"
    ) +
    theme_minimal(base_size = 12) +
    theme(legend.position = "bottom", plot.title = element_text(face = "bold"))
...
if (isTRUE(run_ov_over_time_plot)) {
  write_ov_over_time_plot()
}

if (isTRUE(run_brazil_exploration)) {
  write_brazil_exploration()
}
